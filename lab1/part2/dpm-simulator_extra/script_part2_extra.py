...
with open('Energy_w_dpm/dpm_energy_extra_wl.txt') as temp_f:   #for each line in the file that contains pair TIMEOUT, ENERGY W DPM
    ...
    for line in datafile:
        ...
        x1.append(value_list[0])         #save in x1 value of timeout_idle
        x2.append(value_list[1])         #save in x2 value for timeout_sleep
...
x1 = np.array(x1)     #transform them from list to array (maybe useless)
x2 = np.array(x2)
y1 = np.array(y1)
y2 = np.array(y2)
