...
file = open("simulator_results/results_extra_w2.txt","w+")
...
for i in range (50):
    for j in range (100):
        ...
        stream = os.popen('./dpm_simulator -t ' + str(i) + str(j) + ' -psm example/psm.txt -wl workloads/workload_2.txt')
...
file_savings = open("Energy_w_dpm/dpm_energy_extra_w2.txt","w+")

#file_savings.write("timeout value,  Energy w DPM \n")
min_energy = 0
with open('simulator_results/results_extra_w2.txt') as temp_f:
...
file_best = open("best_result/best_dpm_extra_w2.txt","w")
...
n = 0
with open('Energy_w_dpm/dpm_energy_extra_w2.txt') as temp_f:   #for each line in the file that contains pair TIMEOUT, ENERGY W DPM
