...
j = 0
with open('Energy_w_dpm/dpm_energy_idle_w0.txt') as temp_f2:   #for each line in the file that contains pair TIMEOUT, ENERGY W DPM
    datafile2 = temp_f2.readlines()           #We copy the pair respectively in x and y
    for line2 in datafile2:
        value_list2 = line2.split(",")
        if ((j%10) == 0):                   #add a value every 10 to reduce overhead in the printed figure
...
#plt.show()
plt.savefig('printed_graphs/w0_difference.png')