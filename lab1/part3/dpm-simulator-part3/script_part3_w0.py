...
file = open("simulator_results/results_w0.txt","w+")
...
for i in range (800):
    stream = os.popen('./dpm_simulator -h ' + str(i) + ' -psm example/psm.txt -wl example/wl.txt')
...
file_savings = open("Energy_w_dpm/dpm_energy_w0.txt","w+")

#file_savings.write("timeout value,  Energy w DPM \n")

with open('simulator_results/results_w0.txt') as temp_f:
...
print("Best DPM threshold=%d, Energy w DPM= %f" % (min_pos , min_value))

file_best = open("best_result/best_dpm_w0.txt","w")

file_best.write("Best DPM for the associated threshold value\n : "+str(min_pos)+", Energy consumed: " +str(min_value))
...
n = 0
with open('Energy_w_dpm/dpm_energy_w0.txt') as temp_f:   #for each line in the file that contains pair Threshold, ENERGY W DPM
    datafile = temp_f.readlines()           #We copy the pair respectively in x and y
    for line in datafile:
        value_list = line.split(",")
        if ((n%20) == 0):                   #add a value every 10 to reduce overhead in the printed figure
            x.append(float(value_list[0]))         #save in x value of timeout 
...
plt.axvline(x_point, 0, 1, color='red', label='Tbe', linestyle="--")
plt.plot(x, y1)
...
plt.savefig('printed_graphs/dpm_w0.png')
...
plt.savefig('printed_graphs/saved_energy_w0.png')
