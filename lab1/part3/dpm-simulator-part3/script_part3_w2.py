...
for i in range (400):
    stream = os.popen('./dpm_simulator -h ' + str(i) + ' -psm example/psm.txt -wl workloads/workload_2.txt')
...
file_savings = open("Energy_w_dpm/dpm_energy_w2.txt","w+")
...
n = 0
with open('Energy_w_dpm/dpm_energy_w2.txt') as temp_f:   #for each line in the file that contains pair Threshold, ENERGY W DPM
...
plt.axvline(x_point, 0, 1, color='red', label='Tbe', linestyle="--") #reference line for Tbe
...
plt.axvline(x_point, 0, 100, color='red', label='Tbe', linestyle="--") #reference line for Tbe
...
plt.savefig('printed_graphs/saved_energy_w2.png')
