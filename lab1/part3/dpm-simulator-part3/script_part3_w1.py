...
for i in range (400):
    stream = os.popen('./dpm_simulator -h ' + str(i) + ' -psm example/psm.txt -wl workloads/workload_1.txt')
...
file_savings = open("Energy_w_dpm/dpm_energy_w1.txt","w+")

#file_savings.write("timeout value,  Energy w DPM \n")

with open('simulator_results/results_w1.txt') as temp_f:
...
file_best = open("best_result/best_dpm_w1.txt","w")
...
#x_point=73 #at TBE
#y_point= savings.index[x_point]

n = 0
with open('Energy_w_dpm/dpm_energy_w1.txt') as temp_f:   #for each line in the file that contains pair Threshold, ENERGY W DPM
